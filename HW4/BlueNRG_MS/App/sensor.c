  ...
  case EVT_VENDOR:
    {
      ...
      case EVT_BLUE_GATT_ATTRIBUTE_MODIFIED:
      {
    	  ...
    	  PRINTF("evt->offset:%d\r\n", evt->offset);
//    	  PRINTF("evt->att_data:%d\r\n", evt->att_data);
    	  for(int i = 0;i<evt->data_length;++i){
    		  PRINTF("evt->att_data[%d]:%d\r\n", i, evt->att_data[i]);
    	  }
    	  if(evt->attr_handle == EnvironmentalCharHandle+1){// match
    		  memcpy(&sf, evt->att_data, sizeof(uint32_t));
    		  if (LSM6DSL_ACC_SetOutputDataRate(&MotionSensor, (float)((unsigned)sf)) != LSM6DSL_OK)
    		  {
    			  PRINTF("QQQQQQQ\r\n");
    		      // Handle error
    		  } else {
    			  PRINTF("match, sf is now %u\r\n", (unsigned)sf);
    		  }
    	  }
    	  PRINTF("EnvironmentalCharHandle is %d\r\n", EnvironmentalCharHandle);
      ...
      case EVT_BLUE_ATT_READ_RESP:
      {
		  ...
		  // pr->conn_handle: The connection handle.
		  // pr->event_data_length: Total length of the received data.
		  // pr->attribute_value: Pointer to the read characteristic value.

		  // Example: Print the read value (assuming it's an integer)
